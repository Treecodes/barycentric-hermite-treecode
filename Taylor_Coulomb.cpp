...
void Comp_Taylor_Coeff(double a[P + 1][P + 1][P + 1], double x, double y, double z,
                       size_t panel_index)
{
  // Setup variables
  ...
  // Base case
  a[0][0][0] = 1.0 / R; // First coefficient is the Greens function itself
  ...
  a[1][0][0] = (ijk2 * tp_x * a[0][0][0]) / s;
  a[0][1][0] = (ijk2 * tp_y * a[0][0][0]) / s;
  a[0][0][1] = (ijk2 * tp_z * a[0][0][0]) / s;
  ...
  for (i = 2; i < Q + 1; i++) {
    ...
    a[i][0][0] = (ijk2 * (tp_x * a[i - 1][0][0]) - ijk1 * (a[i - 2][0][0])) / s;
    a[0][i][0] = (ijk2 * (tp_y * a[0][i - 1][0]) - ijk1 * (a[0][i - 2][0])) / s;
    a[0][0][i] = (ijk2 * (tp_z * a[0][0][i - 1]) - ijk1 * (a[0][0][i - 2])) / s;
  }
  ...
  a[1][1][0] = (ijk2 * (tp_x * a[0][1][0] + tp_y * a[1][0][0])) / s;
  a[1][0][1] = (ijk2 * (tp_x * a[0][0][1] + tp_z * a[1][0][0])) / s;
  a[0][1][1] = (ijk2 * (tp_y * a[0][0][1] + tp_z * a[0][1][0])) / s;
  ...
  for (i = 2; i + 1 <= Q; i++) {
    ...
    a[i][1][0] = (ijk2 * (tp_x * a[i - 1][1][0] + tp_y * a[i][0][0])
                  - ijk1 * (a[i - 2][1][0])) / s;

    a[i][0][1] = (ijk2 * (tp_x * a[i - 1][0][1] + tp_z * a[i][0][0])
                  - ijk1 * (a[i - 2][0][1])) / s;

    a[1][i][0] = (ijk2 * (tp_x * a[0][i][0] + tp_y * a[1][i - 1][0])
                  - ijk1 * (a[1][i - 2][0])) / s;

    a[0][i][1] = (ijk2 * (tp_y * a[0][i - 1][1] + tp_z * a[0][i][0])
                  - ijk1 * (a[0][i - 2][1])) / s;

    a[1][0][i] = (ijk2 * (tp_x * a[0][0][i] + tp_z * a[1][0][i - 1])
                  - ijk1 * (a[1][0][i - 2])) / s;

    a[0][1][i] = (ijk2 * (tp_y * a[0][0][i] + tp_z * a[0][1][i - 1])
                  - ijk1 * (a[0][1][i - 2])) / s;
  }
  ...
  for (i = 2; i <= Q - 2; i++) {
    for (j = 2; i + j <= Q; j++) {
      ...
      a[i][j][0] = (ijk2 * (tp_x * a[i - 1][j][0] + tp_y * a[i][j - 1][0])
                    - ijk1 * (a[i - 2][j][0] + a[i][j - 2][0])) / s;

      a[i][0][j] = (ijk2 * (tp_x * a[i - 1][0][j] + tp_z * a[i][0][j - 1])
                    - ijk1 * (a[i - 2][0][j] + a[i][0][j - 2])) / s;

      a[0][i][j] = (ijk2 * (tp_y * a[0][i - 1][j] + tp_z * a[0][i][j - 1])
                    - ijk1 * (a[0][i - 2][j] + a[0][i][j - 2])) / s;
    }
  }
  ...
  ijk2 = 2.0 * sum_ijk - 1.0;
  ijk1 = sum_ijk - 1.0;
  a[1][1][1] = (ijk2 * (tp_x * a[0][1][1] + tp_y * a[1][0][1] + tp_z * a[1][1][0])) / s;
  ...
  for (i = 2; i + 2 <= Q; i++) {
    ...
    a[i][1][1] = (ijk2 * (tp_x * a[i - 1][1][1] + tp_y * a[i][0][1] + tp_z * a[i][1][0])
                  - ijk1 * (a[i - 2][1][1])) / s;

    a[1][i][1] = (ijk2 * (tp_x * a[0][i][1] + tp_y * a[1][i - 1][1] + tp_z * a[1][i][0])
                  - ijk1 * (a[1][i - 2][1])) / s;

    a[1][1][i] = (ijk2 * (tp_x * a[0][1][i] + tp_y * a[1][0][i] + tp_z * a[1][1][i - 1])
                  - ijk1 * (a[1][1][i - 2])) / s;
  }
  ...
  for (i = 2; i <= Q - 3; i++) {
    for (j = 2; i + j < Q; j++) {
      ...
      a[i][j][1] = (ijk2 * (tp_x * a[i - 1][j][1] + tp_y * a[i][j - 1][1] + tp_z * a[i][j][0])
                    - ijk1 * (a[i - 2][j][1] + a[i][j - 2][1])) / s;
      a[i][1][j] = (ijk2 * (tp_x * a[i - 1][1][j] + tp_y * a[i][0][j] + tp_z * a[i][1][j - 1])
                    - ijk1 * (a[i - 2][1][j] + a[i][1][j - 2])) / s;
      a[1][i][j] = (ijk2 * (tp_x * a[0][i][j] + tp_y * a[1][i - 1][j] + tp_z * a[1][i][j - 1])
                    - ijk1 * (a[1][i - 2][j] + a[1][i][j - 2])) / s;
    }
  }
  ...
  for (i = 2; i <= Q - 4; i++) {
    for (j = 2; i + j <= Q - 2; j++) {
      for (k = 2; i + j + k <= Q; k++) {
        ...
        a[i][j][k] = (ijk2 * (tp_x * a[i - 1][j][k] + tp_y * a[i][j - 1][k] + tp_z * a[i][j][k - 1])
                      - ijk1 * (a[i - 2][j][k] + a[i][j - 2][k] + a[i][j][k - 2])) / s;
      }
    }
  }
}
  ...
  for (int i = 0; i < P + 1; i++) {
    for (int j = 0; j < P + 1 - i; j++) {
      for (int k = 0; k < P + 1 - i - j; k++) {
        kk++;
        velocity += a[i][j][k] * tree[panel_index].moments[kk];
      }
    }
  }
