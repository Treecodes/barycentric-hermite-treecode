...
double minval(double* x)
{
  ...
  MinVal = MinVal; // Avoid interpolation points same as particles
...
double maxval(double* x)
{
  ...
  MaxVal = MaxVal; // Avoid interpolation points same as particles
...
void Panel_Moment_B(size_t panel_index, double m[Pflat])
{
    ...
    for (tp_j = tp0; tp_j <= tp1; tp_j++) {
        ...
        D = 1.0 / (sum1 * sum2 * sum3);
        kk = -1;
        for (i = 0; i < P + 1; i++) {
            for (j = 0; j < P + 1; j++) {
                for (k = 0; k < P + 1; k++) {
                    kk++;
                    m[kk] += a1i[i] * a2j[j] * a3k[k] * lam * D;
                }
            }
        }
    }
  ...
  for (int i = 0; i < torder + 1; i++) {
    ...
    for (int j = 0; j < torder + 1; j++) {
      ...
      for (int k = 0; k < torder + 1; k++) {
        ...
        velocity += exp(-kappa * R) * Rinv * temp_moments;
      }
    }
  }
