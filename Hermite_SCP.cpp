...
double Call_Treecode(double x, double y, double z, int panel_index)
{
  ...
  double d1, d2, d3;
  double R2, R, Rinv, Rinv2, Rinv3, Rinv4, Rinv5, Rinv6, Rinv7;
  double expkr;
  ...
  ka2 = kappa * kappa;
  ka3 = ka2 * kappa;
  double d1e, d2e, d3e, d12e, d13e, d23e, d123e;
  double temp, temp1;
  ...
  for (i = 0; i < P + 1; i++) {
    ...
    for (j = 0; j < P + 1; j++) {
      ...
      for (k = 0; k < P + 1; k++) {
        ...
        d13e = d1 * d3 * expkr;
        d23e = d2 * d3 * expkr;
        temp = (3.0 * kappa * Rinv4 + ka2 * Rinv3 + 3.0 * Rinv5);

        ap12 = d12e * temp;
        ap23 = d23e * temp;
        ap13 = d13e * temp;

        d123e = d1 * d2 * d3 * expkr;
        ap123 = d123e * (15.0 * kappa * Rinv6 + 6.0 * ka2 * Rinv5 + ka3 * Rinv4 + 15.0 * Rinv7);
